...
class CUTVEFModel(BaseModel):
    """ 
    CUT + VEF model (without stability regularization)
    ...
    def modify_commandline_options(parser, is_train=True):
        """Configure options specific for CUT + VEF model"""

        # ===== model identity (CRITICAL) =====
        parser.set_defaults(model='cut_vef')
        ...
        parser.add_argument(
            ...
            const=True,
            default=False,
            help='include negatives from other samples in minibatch'
        )
        parser.add_argument(
            '--netF',
            type=str,
        ...
        parser.add_argument(
            ...
            const=True,
            default=False,
            help='flip-equivariance regularization (FastCUT only)'
        )

        # CUT does not use image buffer
